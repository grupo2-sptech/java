...
services:
  ...
  java-app-historico:
    ...
    ports:
      - "8080:8080"
    command: java -jar /app-cliente-historico.jar
    network_mode: host
...
if [[ "$resposta" =~ ^[Ss]$ ]]
then
    sudo docker exec -it java-app-historico java -jar /app-cliente-historico.jar
else
    echo "Para inicializar a aplicação use esse comando: docker exec -it java-app-historico java -jar /app-cliente-historico"
fi
